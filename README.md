...
keeps random access cheap via a 256³ chunk atom with 6 built-in LODs.
...
Chunks stand alone. c3d has no codec-level container above the chunk —
callers wire their own addressing (tiled arrays, directory layouts, object
stores) and hand chunk bytes to the library.
...
Sparse chunks: uniformly-zero 256³ chunks encode to a small constant and
decode to a memset. Callers can also skip encoding known-empty regions
entirely and reconstruct zeros on demand — 75-85 % of masked scroll
chunks fall into this category.
...
for (chunk : chunks) {
    size_t n = c3d_encoder_chunk_encode(e, chunk.in, 25.0f,
                                        chunk.out, max_sz);
    c3d_decoder_chunk_decode(d, chunk.out, n, chunk.dec);
}
...
c3d_decoder_chunk_decode(dec, bytes, n, out);

/* LOD serving (e.g. multiscale zarr groups) */
uint8_t *buf = malloc(c3d_voxels_per_lod(lod));
c3d_decoder_chunk_decode_lod(dec, bytes, n, lod, buf);
```
...
| `c3d_labels_inspect` | dump label chunk + schema metadata               |
| `c3d_labels_codec`   | round-trip label chunk encode/decode             |
